...
class GroceryListWidgets{
                                        ...
                                        Column(
                                          children: [
                                            Form(
                                              ...
                                              child:
                                              TextFormField(
                                                controller: FormController.nom_aliment_controller,
                                                ...
                                                validator: (value) => ErrorHandling().errorEmptyField(FormController.nom_aliment_controller.text),
                                              ),
                                            ),
                                            TextField(
                                              controller: FormController.quantite_aliment_controller,
                                              cursorColor: Colors.redAccent,
                                            ...
                                            TextField(
                                              controller: FormController.descr_aliment_controller,
                                              cursorColor: Colors.redAccent,
                                              keyboardType: TextInputType.name,
                                              maxLength: 16,
                                              decoration: InputDecoration(
                                            ...
                                            ElevatedButton(
                                                ...
                                                onPressed: (){
                                                  AppService().removeImage(setState);
                                                },
                                                child: Row(
                                                  children: [
  ...
  void addFoodButton(BuildContext context) async {
    if(_foodNameFormKey.currentState!.validate()){
      try{
        Member member = Member(DateTime.now(),5,'A1AU6adC6H9d3777lIO1','KkWXYpXzoPYhKVfwmAn56duScUr2','','','');
        Food food = Food(member.id, DateTime.now().toUtc(),FormController.descr_aliment_controller.text, '', '', false, FormController.nom_aliment_controller.text, int.parse(FormController.quantite_aliment_controller.text));
        if(uploadedImage != null){
          await FirebaseFoodService().addFoodToGrocery(food, imageToUpload!, member, context);
        }
      }finally{
        Navigator.of(context).pop();
      }
